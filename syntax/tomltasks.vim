" Vim syntax file
" Language: tomltasks tasks file (TOML + {{ … }} expression holes)
"
" This is the tasks-file filetype's highlighting. It deliberately uses Vim's
" regex engine (no treesitter): tomltasks registers `tasks.toml` as its own
" `tomltasks` filetype, for which no treesitter parser exists, so highlighting
...
" top of that, string values additionally highlight tomltasks expression holes
" (`{{ name }}`, `{{ name(args) }}`, `..` concat, `$1` params, verbatim string
" literals) via the `tomltasksExpr*` groups. See lua/tomltasks/util/expr.lua
...
syn keyword tomltasksExprBool true false contained
syn match   tomltasksExprParam  /\$\d\+/ contained
syn match   tomltasksExprNumber /-\=\d\+\%(\.\d\+\)\=/ contained
syn match   tomltasksExprConcat /\.\./ contained
syn match   tomltasksExprPunct  /[(),]/ contained
" A callee name (bare ident is a zero-arg call). `true`/`false` are caught by
" the keyword above, which outranks this match.
syn match   tomltasksExprFunc   /\<[[:alpha:]][[:alnum:]_-]*\>/ contained
" Verbatim string-literal arguments — both delimiters are accepted (see _delims
" in expr.lua). No escapes inside.
syn region  tomltasksExprString start=/'/ end=/'/ contained keepend
syn region  tomltasksExprString start=/"/ end=/"/ contained keepend

syn cluster tomltasksExprBody contains=tomltasksExprBool,tomltasksExprParam,tomltasksExprNumber,tomltasksExprConcat,tomltasksExprPunct,tomltasksExprFunc,tomltasksExprString
...
syn region  tomltasksExpr matchgroup=tomltasksExprDelim start=/{{/ end=/}}/ contained contains=@tomltasksExprBody

" Escape for a literal `{{`; defined last so it outranks the hole opener.
syn match   tomltasksExprEscape /{{{{/ contained

syn cluster tomltasksExprCluster contains=tomltasksExpr,tomltasksExprEscape
...
" Basic strings
syn region tomlString oneline start=/"/ skip=/\\\\\|\\"/ end=/"/ keepend contains=tomlEscape,@tomltasksExprCluster
" Multi-line basic strings
syn region tomlString start=/"""/ end=/"""/ keepend contains=tomlEscape,tomlLineEscape,@tomltasksExprCluster
" Literal strings
syn region tomlString oneline start=/'/ end=/'/ keepend contains=@tomltasksExprCluster
" Multi-line literal strings
syn region tomlString start=/'''/ end=/'''/ keepend contains=@tomltasksExprCluster
...
hi def link tomltasksExprDelim  Special
hi def link tomltasksExprEscape SpecialChar
hi def link tomltasksExprFunc   Identifier
hi def link tomltasksExprParam  Identifier
hi def link tomltasksExprString String
hi def link tomltasksExprNumber Number
hi def link tomltasksExprBool   Boolean
hi def link tomltasksExprConcat Operator
hi def link tomltasksExprPunct  Delimiter

syn sync minlines=500
let b:current_syntax = 'tomltasks'
