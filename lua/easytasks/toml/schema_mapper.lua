local M = {}

local utils = require('easytasks.toml.validatorutils')
local validator = require('easytasks.toml.validator')

local EXCLUDED_KEYS = { "if", "then", "else", "allOf", "oneOf" }

---@param schema_map table<string, table>
---@param path string
---@param schema table
---@param data any
local function _build(schema_map, path, schema, data)
    local map = schema_map[path] or {}
    utils.deep_merge_tables(map, schema)
    schema_map[path] = map
    for _, key in ipairs(EXCLUDED_KEYS) do
        map[key] = nil
    end

    if schema.type == "object" or (type(schema.type) == "table" and vim.tbl_contains(schema.type, "object")) then
        if type(data) == "table" and not vim.islist(data) then
            local props = schema.properties or {}
            local pattern_props = schema.patternProperties or {}

            for key, subschema in pairs(props) do
                if data[key] ~= nil then
                    _build(schema_map, utils.join_path(path, key), subschema, data[key])
                end
            end

            local addl = schema.additionalProperties
            for key, value in pairs(data) do
                local handled = props[key] ~= nil
                for pattern, subschema in pairs(pattern_props) do
                    if type(key) == "string" and key:match(pattern) then
                        handled = true
                        _build(schema_map, utils.join_path(path, key), subschema, value)
                    end
                end
                if not handled and type(addl) == "table" then
                    _build(schema_map, utils.join_path(path, key), addl, value)
                end
            end
        end
    end

    if schema.type == "array" or (type(schema.type) == "table" and vim.tbl_contains(schema.type, "array")) then
        if vim.islist(data) and schema.items then
            for i, value in ipairs(data) do
                _build(schema_map, utils.join_path(path, tostring(i)), schema.items, value)
            end
        end
    end

    if schema["if"] then
        local ok = validator.validate(schema["if"], data)
        if ok then
            if schema["then"] then
                _build(schema_map, path, schema["then"], data)
            end
        else
            if schema["else"] then
                _build(schema_map, path, schema["else"], data)
            end
        end
    end

    if schema.allOf then
        for _, sub in ipairs(schema.allOf) do
            _build(schema_map, path, sub, data)
        end
    end

    if schema.oneOf then
        local best_sub = nil
        local best_count = math.huge
        for _, sub in ipairs(schema.oneOf) do
            local _, errors = validator.validate(sub, data)
            local count = #errors
            if count < best_count then
                best_count = count
                best_sub = sub
            end
            if best_count == 0 then break end
        end
        if best_sub then
            _build(schema_map, path, best_sub, data)
        end
    end
end

---@param schema table
---@param data any
---@return table<string, table>
function M.build(schema, data)
    local schema_map = {}
    _build(schema_map, "/", schema, data)
    return schema_map
end

return M
