...
describe("string literals (verbatim)", function()
    it("takes backtick contents literally, including quotes", function()
        assert.equal("printf 'a, b'", ast("shell(`printf 'a, b'`)").args[1].value)
    end)

    it("takes double-quoted contents literally, including a backtick", function()
        assert.equal("a`b", ast('x("a`b")').args[1].value)
    end)
